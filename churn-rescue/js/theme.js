/* Theme toggle. Two modes only: light (paper, default) and crt
   (retro black + orange neon). Persists to localStorage; the 't'
   shortcut toggles mid-lecture. Hash override via #theme=light|crt
   for headless verification. Any unrecognised stored value (e.g. a
   legacy 'dark' / 'gb') falls back to 'light' so old localStorage
   entries still resolve safely. */
(function () {
  const STORAGE_KEY = 'churn-rescue.theme';
