/* Value iteration over the 25-state Churn Rescue MDP.
 *
 *   V(s) = max_a { Σ p(s'|s,a) · [ R(s,a,s') + γ · V(s') ] }    for s non-terminal
 *   V(RENEWED) = 0   (absorbing; the +20 lump was already paid on entry)
 *   V(CHURNED) = 0   (absorbing; the -20 lump was already paid on entry)
 *
 * Synchronous backups; 25 states (tier x months-left); 3 levers; cap at
 * 100 iters; tol 1e-4. The MDP is finite-horizon and acyclic in m, so it
 * converges in a handful of sweeps. The reward is baked into successors
 * (-cost per month, plus the +20 / -20 terminal lump) so the backup is
 * just max_a Σ p · (r + γ · V(s')). γ = 1. */
(function () {
