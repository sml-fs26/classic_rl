/* SFX: tiny synthesized sound effects via Web Audio. No asset files, so it
   ...
   Cues (recycling-robot flavored):
     cursor   menu blip
     tick     typewriter tick
     diceroll the drain die tumbling
     spark    a battery pip draining (search)
     trash    +trash collected ping
     dock     RECHARGE dock-and-refill sweep
     strand   the robot powers down -> rescue alarm (the -10) */
(function () {
  let enabled = false;
  let ctx = null;
  ...
  const CUES = {
    ...
    diceroll: (t) => { for (let i = 0; i < 4; i++) tone(280 + i * 90, t + i * 0.04, 0.03, 'square', 0.08); },
    spark:    (t) => { sweep(900, 300, t, 0.14, 'triangle', 0.12); noiseHit(t, 0.05, 0.05); },
    trash:    (t) => { tone(784, t, 0.06, 'square', 0.13); tone(1047, t + 0.05, 0.10, 'square', 0.13); },
    dock:     (t) => { [392, 523, 659, 784].forEach((f, i) => tone(f, t + i * 0.06, 0.10, 'square', 0.12)); },
    strand:   (t) => { sweep(420, 90, t, 0.32, 'sawtooth', 0.18); [233, 175].forEach((f, i) => tone(f, t + 0.18 + i * 0.16, 0.20, 'sawtooth', 0.13)); noiseHit(t, 0.22, 0.09); },
    /* legacy aliases (parity with sibling cue names) */
