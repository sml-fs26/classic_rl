...
(function () {
  ...
  const learner = D.learner || { snapshots: [], valueCurve: [] };
  const QSTAR = D.Qstar || [];
  const optStart = (learner.optimalStartValue != null) ? learner.optimalStartValue : (D.V ? D.V[0] : 9.14);

  function argmaxIdx(Q, st) {
    const base = st * A; let m = -Infinity, k = -1;
  ...
  function agreementPct(Q) {
    let ok = 0;
    for (let st = 0; st < N; st++) { if (argmaxIdx(Q, st) === argmaxIdx(QSTAR, st)) ok++; }
    return 100 * ok / N;
  }
  ...
  window.scenes.scene11 = function (root) {
    ...
    root.innerHTML =
      ...
      '<div class="cs-btn-row sc11-nav">' +
        '<span class="sc11-stepcount muted"></span>' +
        '<button class="cs-btn sc11-back" type="button">' + T('scene11.back') + '</button>' +
        '<button class="cs-btn primary sc11-next" type="button">' + T('scene11.next') + '</button>' +
      '</div>';

    const stage = root.querySelector('#sc11-stage');
    let cursor = 0;
    let learnGrid = null, oracleGrid = null;
    let snapIdx = 0, playing = false, playTimer = null;
    const maxSnaps = learner.snapshots.length;
    ...
    function buildStep1() {
      stage.innerHTML =
        ...
        '<div class="sc11-arrow-down">↓ ' + T('scene11.s1.replace') + '</div>' +
    ...
    function buildStep2() {
      stage.innerHTML =
        ...
        '<div class="cs-btn-row sc11-ctrls">' +
          '<button class="cs-btn primary sc11-play" type="button">' + T('scene11.play') + '</button>' +
          '<button class="cs-btn sc11-reset" type="button">' + T('scene11.reset') + '</button>' +
        ...
        '<div class="sc11-boards">' +
          '<div class="sc11-board sc11-board-learn">' +
            ...
            '<div class="sc11-grid-host sc11-learn-host"></div>' +
          ...
          '<div class="sc11-board sc11-board-oracle">' +
            ...
            '<div class="sc11-grid-host sc11-oracle-host"></div>' +
            '<div class="sc11-readout">' +
              '<div class="sc11-ro-row"><span class="sc11-ro-lab">' + T('scene11.value') + '</span><span class="sc11-ro-val tnum">' + optStart.toFixed(2) + '</span></div>' +
            '</div>' +
          '</div>' +
        '</div>' +
        '<div class="poke-box sc11-framing">' + T('scene11.framing') + '</div>';

      learnGrid  = window.Grid.mount(stage.querySelector('.sc11-learn-host'),  { variant: 'qtable', legend: false });
      oracleGrid = window.Grid.mount(stage.querySelector('.sc11-oracle-host'), { variant: 'qtable', legend: false });
      oracleGrid.update(QSTAR.map(v => (v == null ? null : v)), { suppressFlash: true });
    ...
    function applySnap(i, suppress) {
      ...
      const snap = learner.snapshots[snapIdx];
      if (!snap || !learnGrid) return;
      const Q = snap.Q.map(v => (v == null ? null : v));
      learnGrid.update(Q, { suppressFlash: !!suppress });
      ...
      const vv = stage.querySelector('#sc11-val'); if (vv) vv.textContent = snap.episode > 0 ? (val.toFixed(2) + ' / ' + optStart.toFixed(2)) : '--';
      const epEl = stage.querySelector('#sc11-ep'); if (epEl) epEl.textContent = snap.episode.toLocaleString('en-US');
    }
    ...
    root.querySelector('.sc11-next').addEventListener('click', () => {
      ...
      else window.CS && window.CS.goTo(12);
    });
