/* Click-step scene engine for Critical Spare.
 *
 ...
 *   Cold entry must work: a scene reconstructs from window.DATA / window.Machine,
 *   so deep-linking #scene=N lands correctly.
 *
 *   Hash routing: #scene=N is bidirectional (hashchange). &run is a global flag
 *   (window.CS.run) animation scenes read to auto-trigger their primary button
 ...
 *   scene that teaches them. */
(function () {
  const SCENES = [
    { key: 'scene0',  titleKey: 'scene0.title',  title: 'Critical Spare',            music: 'title'    },
  ...
  const BADGE_STORE_KEY = 'cs-badges';
  ...
  function init() {
    ...
    document.addEventListener('click', (e) => {
      const t = e.target;
      if (t && t.classList && (t.classList.contains('cs-btn') || t.classList.contains('poke-btn')) && !t.disabled) cursorBlip();
    });
