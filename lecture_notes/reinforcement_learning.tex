...
(\sa{service} is the ``repair'' of the title). Rewards are weekly net money, in plain units. The numbers governing \sa{run} are:
\begin{center}
\begin{tabular}{lcc}
\toprule
...
deliveries, and the wear often improves; from \sa{worn}, the van returns to \sa{healthy} $95\%$ of the
time. Under \sa{replace} the reward is $-130$, and the next week starts with a
...
most reward. A policy achieving this maximum is called \emph{optimal}. Note the contrast with $G_i$ itself: $\E[G_i]$ is a single real number that does
not depend on the trajectory, because the averaging has removed the randomness. Therefore
...
play $a$ there, acting optimally afterwards. And while $\E[G_i]$ is a single number, $Q^\star$ is a whole table of numbers, one per
state-action pair: for the fleet, $4 \times 3 = 12$ entries.
\end{remark}
...
accumulated from week $i+1$ on. One week of experience
$(s_i,\, a_i,\, r_i,\, s_{i+1},\, a_{i+1})$, one logbook line, therefore reports: ``this
...
\emph{learning rate}, $0.15$ in the visualization. Never overwrite, only nudge: one lucky
week must not erase a year of evidence. Third, the punchline. The Bellman
...
recipe for a running average (Appendix~\ref{app:runavg} derives it;
Appendix~\ref{app:rm} states the general principle). Set \eqref{eq:sarsa}
...
held constant, like $\alpha$) the call is instead drawn at random, because a cell that is
never visited is never improved: you cannot learn the value of servicing if you never
service. The $a_{i+1}$ in the update is the action \emph{actually} chosen at $s_{i+1}$ by
...
interactive \vizhref{visualization}: slow it down and watch one week nudge one cell.} One
...
$Q^\star$ values and keep jittering. But the order within each row, which is all the
...
guess forever. The choice $\alpha_n = 1/n$ satisfies both demands.
...
itself up by its own bootstraps, and making this rigorous took decades. The recipe is due
to Herbert Robbins and Sutton Monro, who published it in 1951.
...
the newcomer and the average so far.
