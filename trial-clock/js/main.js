/* Click-step scene engine for Trial Clock.
 *
 ...
 *   Cold entry must work: a scene reconstructs from window.DATA / window.Trial,
 *   so deep-linking #scene=N lands correctly.
 *
 *   Hash routing: #scene=N is bidirectional (hashchange). &run is a global flag
 *   (window.TC.run) animation scenes read to auto-trigger their primary control
 *   for headless capture; it must NOT auto-run on plain onEnter.
 *
 *   Concept badges (MDP, POL, RTN, Q*, DP, TD) light up on first reaching the
 *   scene that teaches them. Mirrors the gallery siblings' engine exactly. */
(function () {
  const SCENES = [
    { key: 'scene0',  titleKey: 'scene0.title',  title: 'Trial Clock',               music: 'title'    },
  ...
  const BADGE_STORE_KEY = 'tc-badges';
  ...
  function init() {
    ...
    document.addEventListener('click', (e) => {
      const t = e.target;
      if (t && t.classList && (t.classList.contains('tc-btn') || t.classList.contains('poke-btn')) && !t.disabled) cursorBlip();
    });
