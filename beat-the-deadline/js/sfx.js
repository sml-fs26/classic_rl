/* SFX: tiny synthesized sound effects via Web Audio. No asset files, so it
   ...
   Cues: cursor, tick, coinflip, climb, drop, win, loss. */
(function () {
  let enabled = false;
  let ctx = null;
  ...
  const CUES = {
    ...
    /* dock cues */
    roll:    (t) => { noiseHit(t, 0.08, 0.08); tone(520, t + 0.02, 0.03, 'square', 0.06); },           // dice tumble
    arrive:  (t) => { tone(659, t, 0.07, 'square', 0.14); tone(988, t + 0.06, 0.10, 'square', 0.14); }, // pallet slides on (bright)
    safe:    (t) => { tone(880, t, 0.05, 'triangle', 0.10); },                                          // survived the hour
    blown:   (t) => { sweep(440, 90, t, 0.26, 'sawtooth', 0.18); noiseHit(t, 0.16, 0.12); },            // deadline blows (harsh)
    send:    (t) => { sweep(300, 140, t, 0.16, 'square', 0.16); tone(196, t + 0.12, 0.12, 'square', 0.12); noiseHit(t + 0.02, 0.06, 0.06); }, // truck thunk + rolls off
    win:     (t) => { [523, 659, 784, 1047].forEach((f, i) => tone(f, t + i * 0.09, 0.16, 'square', 0.15)); }, // a profitable ship
