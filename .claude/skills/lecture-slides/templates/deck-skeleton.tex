...
% Engine: pdflatex (light theme, Libertinus serif). Compile FROM the slides/
% directory so \input resolves:
%   pdflatex <thisfile>.tex   (run twice for the nav/toc)
% (Only the explicit-request dark Fira variant needs lualatex.)
%
% House rules baked into this skeleton:
%   * MINIMAL TEXT: a few words per slide, no paragraphs, no bullet walls.
%   * A DIAGRAM on essentially every content slide (TikZ, shared styles).
%   * WORDS, not operator notation: no argmax on a slide for managers.
