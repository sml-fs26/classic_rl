...
#
# Headless screenshot regression for the Pipeline Climb viz.
#
# Usage:
...
#
# Output:
#   tests/shots/baseline/sN.png   committed images for each scene 0..12
...
#     from inside pipeline-climb/ does it).
...
if ! curl -fsS "${SERVER_URL}" >/dev/null 2>&1; then
  ...
  echo "       run:  cd pipeline-climb && python3 -m http.server 8765" >&2
  exit 2
fi
