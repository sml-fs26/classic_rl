/* Click-step scene engine for Gambler's Ruin.
 *
 ...
 *   Cold entry must work: a scene reconstructs from window.DATA / window.Gambler,
 *   so deep-linking #scene=N lands correctly.
 *
 *   Hash routing: #scene=N is bidirectional (hashchange). &run is a global flag
 *   (window.GR.run) animation scenes read to auto-trigger their primary button
 *   for headless capture; it must NOT auto-run on plain onEnter.
 *
 *   Concept badges (MDP, POLICY, RETURN, Q*, DP, SARSA) light up on first
 *   reaching the scene that teaches them. */
(function () {
  const SCENES = [
    { key: 'scene0',  titleKey: 'scene0.title',  title: 'Gambler’s Ruin',           music: 'title'    },
  ...
  const BADGE_STORE_KEY = 'gr-badges';
  ...
  function goTo(idx) {
    ...
    /* Re-assert the engine 'scene' class in case a builder clobbered className. */
  ...
  function init() {
    ...
    /*, Language toggle: re-render badge labels + rebuild every cached
       scene so new strings take effect. Unvisited scenes rebuild lazily., */
  ...
  /* Rebuild every cached scene (used on a language toggle). Clears the cached
     scene DOM + state and re-runs the current scene's builder; others rebuild
     lazily on next visit. */
  function rebuildAll() {
