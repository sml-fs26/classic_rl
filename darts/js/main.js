/* Click-step scene engine for the Darts in the Dark viz.

   5 scenes, 0-indexed throughout (hash, array, builders). The original
   "sample mean fails" scene 2 was retired with the bullseye-oscillation
   change, its `key: 'scene3'` etc are unchanged so deep links survive.

   Pattern lifted from the course-viz skill (kmeans-deepdive) and the sibling
   anymal-mdp viz. Each scene file at js/scenes/sceneN.js registers
   ...
   step inside the scene), false to let the driver advance the scene. */
(function () {
  const SCENES = [
    { key: 'scene0', title: 'Aim in the dark' },
    { key: 'scene1', title: 'Throw a dart' },
    { key: 'scene3', title: 'The Robbins-Monro update' },
    { key: 'scene4', title: 'The α trade-off' },
    { key: 'scene5', title: 'Recap' },
  ];
  ...
  window.DartsViz = { goTo, getCurrentScene: () => current, sceneTitles: SCENE_TITLES };
})();
