/* Click-step scene engine for Stale by Sundown.
 *
 ...
 *   Cold entry must work: a scene reconstructs from window.DATA / window.Bakery,
 *   so deep-linking #scene=N lands correctly.
 *
 *   Hash routing: #scene=N is bidirectional (hashchange). &run is a global flag
 *   (window.SBS.run) animation scenes read to auto-trigger their primary button
 *   for headless capture. &instant disables fades. &theme=light|crt.
 *
 *   Concept badges (MDP, POLICY, RETURN, Q*, DP, SARSA) light up on first
 *   reaching the scene that teaches them. */
(function () {
  const SCENES = [
    { key: 'scene0',  titleKey: 'scene0.title',  title: 'Stale by Sundown',         music: 'title'    },
  ...
  const BADGE_STORE_KEY = 'sbs-badges';
  ...
  function updateDots(idx) { document.querySelectorAll('.dot-pager .dot').forEach((dot, i) => dot.classList.toggle('active', i === idx)); }
  ...
  function updateTitle(idx) { const el = document.getElementById('scene-title'); if (el) el.textContent = titleAt(idx); }
  ...
  function goTo(idx) {
    ...
    if (current >= 0) {
      const old = sceneNodes[current]; if (old) old.classList.remove('active');
      const oldState = sceneState[current];
      if (oldState && typeof oldState.onLeave === 'function') { try { oldState.onLeave(); } catch (e) { console.error(e); } }
    }

    const stage = document.getElementById('stage');
    if (!sceneNodes[idx]) {
      const node = document.createElement('div');
      node.className = 'scene'; node.dataset.scene = String(idx);
      stage.appendChild(node); sceneNodes[idx] = node;
    ...
    current = idx;
    if (window.Music && SCENES[idx].music) { try { window.Music.setTrack(SCENES[idx].music); } catch (_e) {} }
    const instant = /[#&?]instant\b/.test(window.location.hash);
    if (instant) { sceneNodes[idx].style.transition = 'none'; sceneNodes[idx].classList.add('active'); }
    else { setTimeout(() => sceneNodes[idx].classList.add('active'), 20); }
    updateDots(idx); updateButtons(idx); updateTitle(idx); syncHash(idx);
  ...
  function init() {
    ...
    document.addEventListener('click', (e) => {
      const t = e.target;
      if (t && t.classList && (t.classList.contains('gr-btn') || t.classList.contains('poke-btn')) && !t.disabled) cursorBlip();
    });
    ...
    function maybeAwardBadge(idx) {
      ...
      earnedBadges[badge] = true; saveBadges(); renderBadgeRow();
    ...
    function refreshSpeakerNotes() {
      ...
      const target = document.getElementById('speaker-notes-content'); if (target) target.innerHTML = html;
    }
    ...
    window.addEventListener('keydown', (e) => {
      ...
      if (e.key === 'ArrowRight') {
        ...
        const handled = st && typeof st.onPrevKey === 'function' && st.onPrevKey();
        if (!handled) goTo(current - 1);
      } else if (e.key === 'n' || e.key === 'N') { e.preventDefault(); toggleSpeakerNotes(); }
      else if (e.key === 'm' || e.key === 'M') { e.preventDefault(); const b = document.getElementById('music-toggle'); if (b) b.click(); }
      else if (e.key === 'Escape') { if (snVisible) toggleSpeakerNotes(); }
    });
  ...
  function rebuildAll() {
    for (let i = 0; i < sceneNodes.length; i++) {
      const node = sceneNodes[i]; if (node && node.parentNode) node.parentNode.removeChild(node);
    }
    sceneNodes.length = 0; sceneState.length = 0;
  ...
  window.SBS = { goTo, getCurrentScene: () => current, sceneCount: SCENES.length, titleAt, rebuildAll, run: RUN };
  /* Back-compat alias for any shared scene idiom that references window.GR. */
  window.GR = window.SBS;
})();
