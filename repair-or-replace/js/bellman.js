/* Value iteration over the 4-state Repair-or-Replace MDP.
 *
 *   V(s) = max_a { Σ p(s'|s,a) · [ R(s,a,s') + γ · V(s') ] }
 *
 * Continuing task (no terminal states), gamma = 0.9. Synchronous backups;
 * 4 states; tol 1e-4 by default (the precompute passes a tighter one). The
 * reward is baked into successors (weekly profit / shop bill / sticker
 * price / breakdown hit) so the backup is just max_a Σ p · (r + γ · V(s')).
 * Reused unchanged from the sibling gallery viz via the window.Battle /
 * window.Moves aliases. */
(function () {
